...
Runbooks are a way to automate routine tasks that are commonly performed. One such task is the setup and installation of windows servers. In this blog, I demonstrate how to set up and install developer dependencies on a windows server using an Octopus Deploy runbook. The runbook can be executed to set up any number of windows machines.
