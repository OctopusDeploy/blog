...
Traditional automated testing is all about verifying your code works. Test Driven Development may encourage a small number of failing tests, but the expectation is still that future work focuses on resolving test failures. So failing tests are a sign of a bug or unimplemented features.

Working with LLMs requires rethinking this approach. LLMs are non-deterministic by design, which means you can not be sure you'll get the same result even with exactly the same inputs. This manifests itself most visibly with LLMs responding with different phrases to convey the same answer. But the more serious concern for developers is that LLMs will sometimes provide incorrect results even when it previously provided correct results with the same inputs.
...
Retrying tests is still good enough to work around non-deterministic LLMs for most tests. But it doesn't quite capture the end user's experience, which will inevitably be that some answers are incorrect, and without the luxury of simply asking the question again until the correct answer is provided. (On a side note, I did experiment with [Tree-of-Thought prompts](https://github.com/dave1010/tree-of-thought-prompting), which is similar to asking the same question multiple times and picking the best response, but couldn't generate reliable outputs with it.)

In order to use tests to more accurately capture the end user's experience, tests need to be run multiple times to generate a useful sample set, and determining the success of failure of the test based on a sufficient ratio of passing results. It is a subtle, but significant, switch in mindset that embraces the reality that LLMs bring uncertainty to any interaction, and it is our role as developers to be confident what we mean by "uncertain".

I used the term "experiment" to refer code that validates application logic based on a passing threshold based on a number of runs. This is distinct from a "test" that passes if one run produces the correct result.
...
This new style of testing means developers can be assured that their example prompts reach a minimum threshold. It also embraces the reality that the threshold can't be 100% and that retrying until success is not representative of the end user experience.
