...
Everything works as it should.  Virtual machines are added into the scale set, they are registered in Octopus Deploy, and the trigger deploys the latest software to the new machines.

At 50,000 feet, everything looks great, but when we zoom in we see a timing problem.
...
Each morning, scaling out the number of virtual machines takes 2x-3x longer than it should.

Hypothetically, they increase their virtual machine count from 5 to 25.  They see this happen:
...
Because of this, they start the scale-out much earlier in the morning to ensure they have enough resources when their customer base comes online.

# Solution Requirements

Waiting over an hour is not acceptable.  Schedules can be tweaked, but what happens when they need to scale because of an unexpected load?  That is when the customer reached out with the question:

> How hard is it to configure Octopus to pause a deployment while a virtual machine scale set is scaling out?
...
Now I have a list of newly created deployment targets AND the specific deployment target to deploy to.  The last piece of the puzzle is a variable run condition.  This is where the new `Contains` filter comes in.
...
As you can see, when you combine Octostache, Output Variables, and Run Conditions together, you can get a very robust deployment process.  The addition of the `Contains` and `Match` filters in Octostache has made that combination even more powerful.  If you have any questions on how to combine all three in your deployment process, reach out to the Customer Solutions team at [advice@octopus.com](mailto:advice@octopus.com).  We'd love to help!
