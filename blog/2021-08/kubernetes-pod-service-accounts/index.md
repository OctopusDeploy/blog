...
## Create a work pool running inside a Kubernetes cluster
...
This can be done either by deploying the [Tentacle image](https://hub.docker.com/r/octopusdeploy/tentacle) hosted in Docker Hub or using [Octopus Deploy Runbooks](https://octopus.com/docs/runbooks). Please refer to [this blog post](https://octopus.com/blog/kubernetes-workers) for a comprehensive instruction on how to use Octopus Deploy to deploy a worker pool to Kubernetes clusters.
...
Similarly to the deployment target created earlier, the deployment steps are required to run on a worker inside the Kubernetes cluster. Please ensure that you select a valid **Worker Pool** for your steps.
![Step's Worker Pool selection](images/step-worker-pool-selection.png)
## Conclusion

In this post, I have demonstrated how you can utilise the **Pod Service Account** authentication mode when creating a **Kubernetes deployment target**. 
