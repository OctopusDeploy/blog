...
## Intro
Before we started to build the Octopus Cloud platform, there was discussion around where it should be hosted. It eventually came down to two choices: Amazon Web Services (AWS) or Microsoft Azure.

At the end of the day, we settled on AWS mainly because the majority of the team have prior experience with the service offerings from AWS, rather than any specific technical advantages between the two cloud providers.

Excited to get started, we jumped head first into the project using Docker and Kubernetes on Windows. This was about eight months ago and, while we had some success, it felt like a constant uphill battle. At the time it felt as though Docker and Kubernetes on Windows was not as mature as Docker and Kubernetes on Linux, and we could not use Linux because Octopus is not written in .NET Core...yet.

The team had to make a difficult decision: carry on with Docker and Kubernetes after spending a good deal of time and effort, or move to a more traditional tried-and-tested method where each customer would have their own EC2 instance. The decision was made to go down the EC2 instance path, for the time being, and ultimately it was based on the reasoning in the blog post [Making sense of MVP (Minimum Viable Product) – and why I prefer Earliest Testable/Usable/Lovable](https://blog.crisp.se/2016/01/25/henrikkniberg/making-sense-of-mvp) by Henrik Kniberg, because we felt as though we were being very strict on ourselves by trying to develop the perfect product from the very beginning.
...
Non-HA Octopus Cloud servers are distributed across our public subnets and are exposed directly to the internet. Future high-availability Octopus Cloud servers would be distributed across our private subnets, and exposed to the internet through an application load balancer.

At the time of writing this blog post, the provisioning of Octopus Cloud servers is possible in the Oregon (us-west-2) region, on t2.medium EC2 instances, and without high availability. However, in the not-too-distant future the Sydney (ap-southeast-2), London (eu-west-2), and Frankfurt (eu-central-1) regions are likely to become available; as well as the ability to migrate Octopus Cloud servers from one region to another, and high availability.
...
The baseline level for a t2.medium is 40%. CPU credits are accumulated while the CPU utilisation is below the baseline level, and consumed while above the baseline level. If your Octopus Cloud server consumes all of its credits, the CPU will not be able to burst above its baseline level.

During our closed alpha, closed beta, and usability sessions we found that, while idle, an Octopus Cloud server uses roughly 1% of its CPU; while performing simultaneous deployments, CPU utilisation was typically below the baseline level.
...
- lowest latency for performance, and
