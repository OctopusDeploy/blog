...
From 3:37pm UTC Wednesday, January 25, 2023 to 3:44pm UTC Thursday, January 26, 2023, customers in some regions couldn't access octopus.com/signin, the blog, the docs, and other URLs, as they returned HTTP 404 (page not found) responses. This mostly affected the US and EU, with some additional intermittent impact in some regions in Asia.

In this post, we detail what happened, our response, and our learnings.

## How the incident started

On January 25, we scheduled maintenance on the `octopus.com` and `octopusdeploy.com` domains as part of a planned DNS and routing provider migration. In this step of the migration, the routing setup in one Azure Front Door (AFD) profiles was transferred to another AFD profile. The new AFD profile didn't behave as expected.

We identified a subset of requests in certain regions were incorrectly routed by the new AFD profile, resulting in HTTP 404 responses. We publicized the service interruption for the scheduled maintenance on our Status Page. 
...
**_07:05 _** A network outage in Azure obscured the issue - see our [incident report](https://octopus.com/blog/cloud-connectivity-disruption-report-learnings).
...
**_15:27 - 17:06_** Investigations showed that the web app and database for the affected routes were not under any significant load. The new AFD profile reported a warning on the `octopus.com` CNAME record.
...
**_02:01_** A new potential root cause identified leading to support request raised with original DNS provider.
...
**_03:12_** Incident was escalated internally to bring in senior engineering management staff due to the length of time passed without a clear path to resolution.
