...
The first and most pressing issue to resolve getting Cypress into your deployment pipeline.
...
The downside to having the tools directly installed in this manner is that you can not easily do it when using dynamic workers in a hosted Octopus instance, and in platforms like Kubernetes there is no concept of a VM for you to preconfigure.
...
However, I suspect most teams that invest in end-to-end testing will want to retain the ability to quickly update their tests scripts without the burden of including them in new Docker images. Wouldn't it be nice to have a common, generic Cypress Docker image that could execute random test scripts?
