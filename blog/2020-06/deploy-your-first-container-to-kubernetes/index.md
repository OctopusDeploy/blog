...
Running `kubectl` locally is great for seasoned Kubernetes administrators, and often required when creating new deployments to debug and verify that things work as expected. It does have some disadvantages though:
...
* Using `kubectl` requires a decent level of familiarity with Kubernetes administration.
...
* The credentials are managed by Octopus, and so only need to be updated in a single location. 
