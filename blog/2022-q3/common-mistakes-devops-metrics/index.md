...
Similarly, the metrics you collect and show will change over time as you respond to differences in the team and organization.

As you create and evolve your measurement system, there are some common mistakes you should avoid.

## Ignoring the data

The first problem with metrics is that great effort is expended to collect them, but they aren't used. This can happen even in organizations that regularly review the numbers.
...
There are four levels of measurement, with activity often being the earliest and easiest to collect. You can usually track activity in near real-time. The result of the activity is not typically available as an output or outcome-based metric until some later date.
...
Activity metrics are usually built-in to your existing tools, so they are already available. The problem is that not all activity represents progress. Some activities might even reduce output and result in worse outcomes.
...
The number of metrics you collect and display can increase, often rapidly. Before long, you have filled your dashboard with charts, and you don't know what is important and what isn't.
...
If you already have a dashboard, open it up and for each chart, ask, 'what would I do differently if this number went up or down?' You can revisit this question frequently, deleting any chart where you don't have an answer.
...
It is easy to be distracted by the task of creating an engaging dashboard. If you don't begin with metric design, you end up with lots of pleasing dashboards that have no impact your daily work. You'll need dashboards and charting tools as they will help you make sense of the information, but design the metrics first.

It is better to start in low-fidelity to collect meaningful metrics. It's okay to start with a simple spreadsheet or even a whiteboard. Once you work out which measurements are currently helpful to your team and organization, you can approach the task of automating the collection and creating slick displays.

If you invest too much time creating a stunning dashboard, you'll find it harder to delete charts when they are no longer required. By spending too much time on a dashboard, you end up making it too permanent.
...
You should address metrics as part of your continuous improvement activities. For a team with long lead times, you would measure batch size, queue times, and the time work spends in each state. These metrics are unsuitable for a team whose primary problem is quality.

This requires the data, insight, theory, experiment cycle where you look at the information you have about the problem you want to solve, form a theory about what might improve the issue, and then run an experiment to test your idea.

The metrics you collect also signal to the team what is important *at the current time*. You'll often see improvements simply because your metrics communicate that you care about some aspect of software delivery.
...
If a team is working to increase their deployment rate, it can be tempting to incentivize them with a reward if they achieve daily deployments. This approach to motivation leads to poor outcomes. A team might let other critical work slip to achieve the goal - not to cheat the system but because you've made daily deployments more important than anything else.
...
Whatever you measure, you need to constantly refine your metrics to ensure they remain useful to your team and organization. Ideally, the metrics you collect are relevant to a specific experiment you are running to test a theory.
