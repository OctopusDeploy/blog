...
Let's break down this POM file to understand the various components that it is comprised of.
...
Here we have set the `project.build.sourceEncoding` property, which is a setting recognized by Maven; defined the versions for the dependencies that our code relies on; and defined the version of Java that our code will be compiled against in the `java.version` property.
...
The property `project.build.sourceEncoding` is set to UTF-8 to specify a consistent encoding of the Java source files across operating systems. If this value was not set, different operating systems assume that the Java files have different encodings, which can cause issues when the code is compiled by different people or services. In later lectures we'll configure an external service based on Linux to build our code for us, and setting this property means our builds will work as expected regardless of what OS we develop the code with locally.

## Plugins

The next settings configure certain aspects of the Maven build process. Maven projects have a well known default life cycle that has the following default phases, as shown in this slide.
...
-   install - install the package into the local repository, for use as a dependency in other projects locally deploy.
...
The configuration of the Java version is done with the `<source>` and `<target>` elements. The value of these elements are both set to `${java.version}`, which is how we reference the value of the custom property called java.version defined in the `<properties>` element
earlier.
...
Clicking on the version number (the 4.12 link in the example above) takes you to the following page with provides the XML that you can add to a `pom.xml` file to include this dependency.

![](./image4.png)

JUnit is a popular unit testing library, and we will use it extensively while writing WebDriver tests. In addition, we add a dependency on the selenium-java library, which provides an implementation of the WebDriver API.
...
Save the `pom.xml` file to a location on your local PC, or you can download a copy of the code with the `pom.xml` file included. Once the file is saved we can import it into IntelliJ, from which we will continue to work on writing our Java code.
...
This dialog allows you to select the JDK to use to build the project. If you are setting up IntelliJ for the first time, this list will be empty.
...
On a MacOS system, the Oracle JDK is usually found under a directory like `/Library/Java/JavaVirtualMachines/jdk1.8.0\_144.jdk/Contents/Home`.
...
The default name for the IntelliJ project is the same as the Maven
ArtifactID. This default is fine, so click the Finish button.
...
Before we wrap this lecture up I want to demonstrate the effect setting the `project.build.sourceEncoding` property has. Comment this line out of the `pom.xml` file and save the changes.
...
Notice that when the `project.build.sourceEncoding` property is not set, Maven displays a warning of 

```
[WARNING] Using platform encoding (UTF-8 actually) to copy filtered resources, i.e. build is platform dependent!
```

when building the project. Revert the change and package the project again, and you will notice that the warning disappears.

![](./image20.png)

And with that we have a minimal Maven project imported into IntelliJ. In the next lecture we will start writing some Java to implement our very first WebDriver test.
