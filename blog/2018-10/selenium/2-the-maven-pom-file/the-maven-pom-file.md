...
The next settings configure certain aspects of the Maven build process. Maven projects have a well known default life cycle that has the following default phases, as shown in this image.
...
Save the `pom.xml` file to a location on your local PC. Once the file is saved we can import it into IntelliJ, from which we will continue to work on writing our Java code.
...
Before we wrap this post up I want to demonstrate the effect setting the `project.build.sourceEncoding` property has. Comment this line out of the `pom.xml` file and save the changes.
