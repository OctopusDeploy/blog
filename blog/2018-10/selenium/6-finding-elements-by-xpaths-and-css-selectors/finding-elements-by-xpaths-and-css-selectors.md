...
Fortunately, Chrome and other browsers provide an easy way to generate unique XPaths for elements. Right clicking on the element in the `Elements` tab and selecting {{Copy,Copy XPath}} will place the most succinct unique XPath that identifies the element into the clipboard. This functionality means you don’t need to understand the nitty gritty details of XPaths in order to make use of them, as you can let the browser take care of generating the XPaths for you. 
...
Note, we call `By.xpath()` instead of `By.id()` in these methods. This is how we search for elements based on their XPath:
...
Like XPaths, all elements in a HTML document have a unique CSS selector they can be identified with.

CSS selectors are the same identifiers used in CSS rule sets. If you have done any web development then you will most likely be familiar CSS selectors. But even if you aren’t, Chrome and other browsers provide a way to generate CSS selectors for HTML elements. In Chrome, right click the element in the `Elements` tab, and select {{Copy,Copy Selector}}.
...
We have now seen three different methods of identifying elements in a HTML page: with the ID, using XPaths, and using CSS selectors. But which way is best?
...
Unfortunately, because the `id` attribute has to be manually assigned to an element, you can not rely on the elements you wish to interact with having `id` attributes. In fact, in my experience it is quite rare to have `id` attributes that can be used when writing tests against real world applications.
...
CSS selectors tend to be more familiar than XPaths, because CSS selectors are used by web developers to design CSS rule sets. For this reason, I recommend CSS selectors over XPaths.
