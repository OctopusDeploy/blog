...
To acomplish this, we will pipe our resource groups to the `where-object` (which is shortened to a `?` to filter each item which is greater-than or equal (`ge`) to the cost notification tag `NotifyCostLimit`
```PowerShell
...
Below is the full script you will need to paste:
