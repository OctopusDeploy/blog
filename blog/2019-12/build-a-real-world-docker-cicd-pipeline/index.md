...
### Craft the build definition
After the project has been created, we can create a new build definition to perform our Docker build operation.  This build definition needs to perform the following steps:
...
Check **Log in to the Docker registry before the build**, choose the connection we created for our project, and click Save:
...
For the push step, we specify all of the images that were built in steps 1-4 and push them in a single step.  Barring any typos, the build should execute successfully:

![](teamcity-build-result.png)

Congratulations!  We just finished the CI portion of this article.  The only thing left to do is add a trigger so that when someone commits to source control, a build will automatically execute.

## Configure Docker continuous delivery with Octopus Deploy
For the continuous delivery portion of this article, we’ll use Octopus Deploy.  With Octopus, we’ll do the following:
...
With our external feed configured, we can define our steps.
...
So far we’ve done the CI and the CD portions, but we still need to connect them.  To fit these pieces together, we go back to our TeamCity server.
...
This will add some steps that we can use with our build definition to interact with Octopus Deploy:
