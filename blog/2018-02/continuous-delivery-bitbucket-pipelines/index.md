---
title: Continuous Delivery for Dotnet Core using Octopus Deploy and Bitbucket Pipelines
description: Learn how to use Bitbucket and Bitbucket Pipelines to automatically build, test and deploy dotnet core apps on code commits using Octopus Deploy
visibility: private
...
1. Bitbucket Pipelines kicks off a build script once the commit is pushed to the remote repository
...
We've got a few accounts and services to create, and instructions for setting all of them up is beyond the scope of this article. The key things you will need are:

### Create a Bitbucket account

Head over to https://bitbucket.org/account/signup/ and set up a Bitbucket account, if you don't already have one. This is where you will be setting up your remote repository and keeping your source code.

### Create a new repository

If you've just created a new Bitbucket account, you should also set up a repository you can push your source code to: the Bitbucket UI is fairly intuitive, or you can [read this useful tutorial](https://confluence.atlassian.com/bitbucket/create-a-git-repository-759857290.html) if you'd prefer specific instructions. Once you've set up the repo, do an initial commit and push to it from your local source code (or whatever toy project you are testing this with) so you've got some code committed.
...
So in a nutshell, this template file will **restore** your Nuget packages, **build** your project and run your **unit tests**. For now, just modify the `export` commands to point to your project and test project names respectively, and use a `#` to comment out any lines that don't apply. 

I've set mine up with a project name variable, used by the `build` command, but I've removed the `TEST_NAME` variable to show how you can comment out lines you don't need, and that you don't _need_ to use variables, if you don't want. My fully configured file looks as so:

![Configured Pipelines YAML file](pipelines-yaml-configured.png)

Then we just need to hit `Commit file` and the pipelines file will be committedc to your repo in the correct place (the root of the project), and your first build will be kicked-off. If everything was configured correctly, you should see a screen like this:
...
# this will publish our app output into a subfolder, so we can package it up
...
# curl comes installed in the image, so we just need to post the contents of the zip file to our Octopus instance, making sure we add a HTTP Header for our API key. This will be expanded by Pipelines from the environment variable we created previously
...
In order for automatic release creation to work, we need to have at least one step that deploys a package: that's how Octopus knows which packages to watch for in the built-in feed. If you have an Azure account set up already, have a read of [this article](https://octopus.com/docs/deploying-applications/azure-deployments/web-apps/getting-started-with-azure-web-apps) for information on setting up an Azure deploy step. Otherwise, consult the Octopus docs for more information on deploying to your environment.
