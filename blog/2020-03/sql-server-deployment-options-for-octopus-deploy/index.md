...
If you have a SQL Server database and you use Octopus Deploy, you have several options with regards to database source control and deployment tooling. This blog post aims to provide a basic overview of the most widely used tools to help you decide which option will best suit your team.

## Model-based and migration-based solutions

Most database source control and deployment tools can be broadly classified as either “model-based” (aka “state-based”) or “migration-based”.

Model-based tools script out the desired end state of your database schema, typically with a CREATE script for each object. At some point within the deployment pipeline, software will compare the source code and the target database and generate a deployment script.
...
In my 10 years working in the database DevOps space I am yet to see a single example of a home-grown solution working better for an organisation than a similar open source or vendor solution. Don’t do it. Use one of the tools in this post instead.

## The most popular tools

It generally pays to use a popular tool. Widely used tools are typically better supported and better documented and often have active communities where answers can be found. It’s also often possible to recruit folks with prior experience using the tools.
...
SSDT is built by Microsoft and comes out of the box with Visual Studio (VS) - so assuming your developers have appropriate MSDN licences it’s essentially free. That makes it the natural solution for most regular VS users. However, for database developers and DBAs who are more familiar using SQL Server Management Studio (SSMS), VS can be pretty confusing.
...
SSDT does have some hybrid features to catch these issues, like the Refactor Log and Pre- and Post-Deployment Scripts, but these often become a pain to manage over time.
...
* [SQL Source Control](https://www.red-gate.com/products/sql-development/sql-source-control/) is a plug-in for SSMS that creates a model-based project for your database.
* The [SQL Change Automation](https://www.red-gate.com/products/sql-development/sql-source-control/) (SCA) VS and SSMS plugins create a migration-based project for your database. However, while SCA projects are primarily migrations-based, they have several powerful hybrid features. This makes SCA projects the truest hybrid migrations- and model-based tool available at the time of writing.
...
Since stored procedures and functions are often a nightmare to manage with a migrations-based tool due to overwrites, all the “programmable objects” are extracted into a separate part of your project where they are treated as a mini model-based subproject. This also helps to reduce the total number of migration-scripts, making the migrations-based part easier to manage. That’s the two biggest issues with migrations-based projects avoided/reduced.

One of the other challenges with migration-based tools is spotting conflicts. SCA offers a handy solution by keeping a “schema model” in another mini subproject, with create scripts for all your tables. If two migration scripts update the same table, you’ll see a merge conflict in the schema model. The schema model also acts as a handy reference point for the desired end state, despite the fact it is not used for deployment.

This hybrid approach does have some flaws. It’s more complicated so it comes with a learning curve, and baselining existing projects isn’t very easy. Also, if you use a stored procedure within any of your migrations scripts you can hit some complicated race conditions with the stored procedure updates always being applied last – but that’s a bit of an edge case for most teams.

Frankly, the biggest issue with SCA is that you need to ask someone for some budget. Unlike all the other tools listed in this blog post, the Redgate tools aren’t free.

**Best thing:** Best hybrid solution, providing the benefits of both a model- and a migrations-based solution  
...
However, the developers tend to get poor feedback about the code that ends up getting pushed to the database. The combination of developers with limited SQL Server experience and poor feedback about the code that gets pushed to SQL Server often results in queries that make DBAs very, very unhappy.

Certainly, ORMs are popular in the .NET community – but approach with caution. And, with my DevOps hat on, make sure that you remember to optimise for the whole value stream, rather than an individual developer or team/silo, at the expense of downstream workers.
...
It’s a Java application, which might seem a little odd for a SQL Server deployment tool, but that’s because, unlike any of the other tools in this blog post, from the start Flyway was designed to be cross-platform. Flyway users could be running MySQL, Postgres, Oracle or any of the other of the 20 or so RDBMSs that Flyway supports. This makes it a popular choice for shops that support multiple relational databases.
...
That said, it does not have a comparison engine built in. That means it can’t generate your deployment scripts for you. You’ll need to create them by hand or by using some other database comparison tool.
...
Like Flyway, DbUp doesn’t come with any clever comparison tooling or fancy features. It’s deliberately a plain, simple migrations runner. All the work creating and managing those upgrade scripts is up to you.

**Best thing:** Simple/customisable, .NET

**Worst thing:** Requires some C# skills

## And the rest…

There’s a long tail of other solutions. Some people might be annoyed I didn’t include more vendor tools like [Idera](https://www.idera.com/productssolutions/it-database-management-tools#DESIGNModelsandDatabases), [ApexSQL](https://www.apexsql.com/) or [DBmaestro](https://www.dbmaestro.com/) or any of the zillions of the other open source migration script runners like [Liquibase](https://www.liquibase.org/), [RoundhousE](https://code.google.com/archive/p/roundhouse/downloads) or [DBDeploy](http://dbdeploy.com/).
