...
As you project the natural evolution of a GitOps workflow, you are likely to conclude that so many automated processes rely on the declarative configuration being in a specific location and format that git commits must be treated in much the same way as a database migration. At this point the interaction with the git repo processing between the git repo and cluster should be automated, rendering much of what we talk about as GitOps today as simply the last hop in a specialized CI/CD pipelines or DevOps workflows.
