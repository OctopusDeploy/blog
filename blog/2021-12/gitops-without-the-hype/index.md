...
GitOps tools also tend assume that because everything is in git, the intent of every change is annotated with commit messages, put through a review process, and is available for future inspection. However, this is overly simplistic, as any process advanced enough to consider implementing GitOps will immediately being iterating on GitOps by automating manual touch points, usually with respect to how configuration is added to the git repo.
