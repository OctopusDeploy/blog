...
Prior to automated database deployments, delta scripts for `Staging` and `Production` were generated by Database Developers or Lead Developers.  The delta scripts would be saved on a file share for the DBAs.  The DBAs would run the script on the appropriate environment.
...
In order for the build a package process to work, the database has to be placed into source control using [Redgate's SQL Source Control](https://documentation.red-gate.com/soc).  In addition, Redgate has a [CLI version](https://documentation.red-gate.com/sc14/using-the-command-line) of their schema compare tool.

One thing I should've done was resolve all the deltas between each environment.  Different users and role membership is expected.  Missing tables, different stored procedures, not so much.  Once they are resolved, then pick an environment to put into source control as your baseline.
...
`10_Dev` and `20_Test` follow the same process.  First I needed to configure the dependencies on the build.  This includes the snapshot dependencies and artifact dependencies.

![](automated-database-deployments-v1-dependencies.png)

Now that I have the package and the dependency configured I can add in the step to sync the `Test` database with the package.  Please note, to deploy the latest package I am overriding build number from the  using the value from the `00_Build` project.  
...
In addition I introduced a couple of new problems.
...
- Latest changes only, can't pick an older version.
...
The trial and error was around steps 1 and 4.  At first I was running the same CLI script from TeamCity to generate the delta script to review.  That wasn't exactly the same as what was getting deployed.  Eventually I learned about the `Create Database Release` and `Deploy from Database Release` step templates provided by Redgate.  
...
The DBAs were more than happy to let Octopus Deploy deploy to `Staging` and `Production`.  They could review the scripts during the deployment.  And who reviewed the scripts was audited.  In addition, they were happy to see all it took was a push of a button.  As one DBA put it "this is all stupid simple."
...
While the DBAs could deploy to `Production`.  But they didn't have rights to change the deployment process.  On Developers could do that.
...
- Latest changes only, can't pick an older version. -> **Solved**
...
- There was no test data in developer's local databases, this resulted in them pushing unfinished changes to `Test`.  They'd then point their code to `Test` so they had data to test with.
...
- DBAs only want to be notified if a deployment to `Production` fails.
...
After going through this process, deployments to `Production` became a non-event.  The DBAs only had to jump online during `Production` deployments when something failed.  Which became more and more rare.  It took several iterations to get there.

What surprised me the most was how much everything changed from start to finish.  I'll be honest, if I came across a customer with the final database deployment process I'd have a lot of questions.  But it makes sense in the context of the company who implemented it.  It meets their requirements.  If you take away anything from this article, it would be this: don't be surprised by the number of iterations you will have to make and where you will eventually end up.  Every company is different.
