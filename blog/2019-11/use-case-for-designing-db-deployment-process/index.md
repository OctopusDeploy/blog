...
- Unable to leverage branches effectively.  There can be 1 to N branches, but only one database.  
...
- DBAs being the bottleneck -> The three teams were deploying to `Staging` so often the DBAs couldn't keep up.  They also got tired of having to be online during deployments so they could approve the deployment to `Production`.  We switched it up so the process generated two files when deploying to `Staging`, one for `Staging`, one for `Production`.  The deployment would occur on `Staging` and they would approve everything after the fact.  We added a step to page them when a deployment failed in `Production`.  
